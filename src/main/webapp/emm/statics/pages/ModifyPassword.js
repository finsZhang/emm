...
function PasswordSubmit(){
    ...
    $.ajax({
        ...
        cache:false,
        async: false,
        url: GLOBAL.WEBROOT + "/emm/PasswordChange.ajax?passwordOld="+passwordOld+"&passwordOldAgain="+passwordOldAgain+"&passwordNew="+passwordNew,
        dataType: 'json',
        success: function (data) {
            if (data.code == "0") {
                layerConfirm('密码修改成功！',function () {
                    window.location="/emm/emm/login.html";
                });
            }
            else {
